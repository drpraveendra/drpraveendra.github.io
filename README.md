# Hi there, I'm Dr. Praveendra Singh 👋

I am an **Assistant Professor of Mathematics** at Government College Kherwara, Rajasthan, India, specializing in Operations Research and Supply Chain Optimization. I am also the creator of educational mathematics content for B.Sc., M.Sc., and competitive exam students.

## 🔭 Current Focus
- **Research:** Developing mathematical models for deteriorating inventory items using nature-inspired metaheuristic algorithms (Differential Evolution, Bat Algorithm).
- **Teaching:** Creating high-quality, animated mathematics lectures in Hindi for university students and CSIR NET/GATE aspirants.
- **Development:** Utilizing Python, Manim, and Scilab to create computational math visualizations.

## 🛠️ Technical & Computational Skills
- **Languages & Tools:** Python, Scilab, LaTeX, Markdown
- **Libraries:** Manim (for mathematical animations), NumPy, SciPy
- **IDEs & Software:** PyCharm, VS Code, Samsung Notes (for digital handwriting)

## 🌐 Connect with Me
- **Website:** [Insert your website link here]
- **YouTube:** [Fractal Frontier Maths / Your Channel Link]
- **Email:** drpraveendra.singh@gmail.com

---
*"Mathematics is the language in which God has written the universe." - Galileo Galilei*
